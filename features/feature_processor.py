...
# Low-cardinality columns we one-hot encode outright (protocol numbers are
# a small fixed set: 6=TCP, 17=UDP, 1=ICMP, etc.)
DEFAULT_ONEHOT_COLUMNS = ["protocol"]


class TargetEncoder:
    """Simple smoothed mean-target encoder for high-cardinality categorical
    columns (e.g. if you later add a categorical like `dst_port_bucket` or
    a service-name feature). Not used by default since `protocol` is
    low-cardinality and one-hot encoded instead -- kept available for
...
class FeatureProcessor:
    def __init__(
        ...
        onehot_columns=None,
        nan_strategy="median",       # "median" | "mean" | "zero" | "drop"
        ...
        self.scaler_ = None
        self.feature_columns_ = []   # final column order after encoding, before scaling
    ...
    def fit_transform(self, df: pd.DataFrame):
        ...
        if self.label_column and self.label_column in df.columns:
            ...
            # keep y as a Series indexed like df, so it survives dropna/dedup
            # row filtering below via a single final .loc[work.index] instead
            # of having to be manually re-sliced at every filtering step.
    ...
    def transform(self, df: pd.DataFrame):
        ...
        # align columns exactly to what the scaler expects (fills missing
        # engineered columns with 0, drops anything unexpected)
    ...
    def _handle_nan_transform(self, df):
        ...
        for col, fill in self.impute_values_.items():
            if col in df.columns:
                df[col] = df[col].fillna(fill)
        # any remaining NaNs (new columns not seen at fit time) -> 0
